...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        const validOption: boolean = question.options.some(
            (option: string): boolean => (answer === option ? true : false),
        );
        return validOption;
    }
}
...
export function toShortForm(question: Question): string {
    const str: string = question.id + ": " + question.name.substring(0, 10);
    return str;
}
